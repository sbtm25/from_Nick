...
if [ "$color_prompt" = yes ]; then
    if [ $(id -u) -eq 0 ]; then
      PS1='\[${BRIGHT}\]\[${CYAN}\]$(__git_ps1)\[${NORMAL}\]${debian_chroot:+($debian_chroot)}\[${BRIGHT}\]\[${RED}\]\u@\[${GREEN}\]\h\[${NORMAL}\]:\[${BRIGHT}\]\[${MAGENTA}\]\w\[$(if [ $ecode -eq 0 ]; then printf $NORMAL; else printf $RED; fi)\]\$\[${NORMAL}\] '
    else
      PS1='\[${BRIGHT}\]\[${CYAN}\]$(__git_ps1)\[${NORMAL}\]${debian_chroot:+($debian_chroot)}\[${BRIGHT}\]\[${GREEN}\]\u@\h\[${NORMAL}\]:\[${BRIGHT}\]\[${MAGENTA}\]\w\[$(if [ $ecode -eq 0 ]; then printf $NORMAL; else printf $RED; fi)\]\$\[${NORMAL}\] '
    fi
else
...
# Change ls dirs to funky colors
